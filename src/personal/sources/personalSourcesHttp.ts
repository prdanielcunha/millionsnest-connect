...
import { RadarComposerTone } from '../radar/personalRadarService';
import { PersonalSourcesService } from './personalSourcesService';
...
function publicCode(error: unknown): string {
  ...
  return code.replace(/[^A-Z0-9_]/g, '') || 'PERSONAL_SOURCES_REQUEST_FAILED';
}
...
function humanSummary(error: unknown): string {
  ...
  if (code === 'RADAR_PILOT_FORBIDDEN') return 'As Fontes Pessoais ainda estão em piloto privado para administração do ecossistema.';
  if (code === 'PERSONAL_VAULT_UNAVAILABLE') return 'Seu cofre pessoal está temporariamente indisponível.';
  if (code === 'SOURCE_NOT_FOUND') return 'Esta conversa importada não foi encontrada no seu cofre pessoal.';
  ...
  if (code === 'IMPORT_FILE_SIZE_INVALID') return 'O arquivo é grande demais. Use uma exportação de até 5 MB.';
  ...
  return 'Não foi possível concluir esta operação das Fontes Pessoais.';
}

export function createPersonalSourcesRouter(service: PersonalSourcesService) {
  ...
  router.get('/radar', execute(async (req, res) => {
    const result = await service.getRadar({ authToken: authToken(req), organizationId: organizationId(req) });
  ...
  router.get('/sources', execute(async (req, res) => {
    const result = await service.listSources({ authToken: authToken(req), organizationId: organizationId(req) });
    return res.status(200).json({ success: true, ...result });
  }));

  router.get('/sources/:sourceId', execute(async (req, res) => {
    const result = await service.getSource(
      { authToken: authToken(req), organizationId: organizationId(req) },
  ...
  router.post('/sources/:sourceId/outreach', express.json({ limit: '32kb' }), execute(async (req, res) => {
    const body = req.body as Record<string, unknown>;
    const result = await service.prepareSourceOutreach(
      { authToken: authToken(req), organizationId: organizationId(req) },
      safeId(req.params.sourceId),
      {
  ...
  router.get('/people/:personId/context', execute(async (req, res) => {
    const result = await service.getPersonContext(
      { authToken: authToken(req), organizationId: organizationId(req) },
  ...
  router.get('/brief', execute(async (req, res) => {
    const result = await service.getBrief({ authToken: authToken(req), organizationId: organizationId(req) });
    return res.status(200).json({ success: true, ...result });
  }));

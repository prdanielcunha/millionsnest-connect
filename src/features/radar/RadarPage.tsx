...
export const RadarPage: React.FC<RadarPageProps> = ({ session, currentLang }) => {
  ...
  const openComposer = (person: RadarClientPerson, signal: RadarClientPerson['signals'][number]) => {
    ...
    const persistedObjective = SALES_STAGES.some(stage => stage.objective === person.salesStage) ? person.salesStage as ComposerObjective : null;
  ...
  const persistCommercialState = async (personId: string, update: Parameters<PersonalRadarClient['updatePerson']>[1]) => {
        ...
        return <div className="fixed inset-0 z-[100] flex items-end bg-black/70 backdrop-blur-sm sm:items-center sm:justify-center" onClick={() => setSelected(null)}>
          <section className="max-h-[92dvh] w-full overflow-y-auto rounded-t-[30px] border border-white/10 bg-[#0d111b] shadow-2xl sm:max-w-3xl sm:rounded-[30px]" onClick={e => e.stopPropagation()}>
            ...
            <div className="space-y-5 p-5">
              <div><div className="text-xs font-semibold text-white">{ui.journey}</div><p className="mt-1 text-xs leading-5 text-slate-500">{ui.journeyHelp}</p><div className="-mx-1 mt-3 flex gap-2 overflow-x-auto px-1 pb-2">{SALES_STAGES.map(stage => <button type="button" key={stage.objective} onClick={() => { setComposerObjective(stage.objective); void persistCommercialState(selected.person.id, { salesStage: stage.objective }); void compose(selected, tone, composerStyle, stage.objective); }} className={`shrink-0 rounded-xl border px-3 py-2 text-left ${composerObjective === stage.objective ? 'border-indigo-400/40 bg-indigo-400/10 text-white' : 'border-white/8 bg-white/[0.025] text-slate-400'}`}><span className="mr-1.5 text-[10px] font-semibold text-indigo-300">{stage.n}</span><span className="text-xs font-medium">{stage.label[currentLang]}</span></button>)}</div></div>
              ...
              <div className="grid gap-2 sm:grid-cols-3"><button type="button" onClick={async () => { await navigator.clipboard.writeText(draft); setCopied(true); if (selected) await persistCommercialState(selected.person.id, { salesStage: composerObjective, commercialAction: 'copied' }); }} disabled={!draft} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-white/10 px-3 text-sm font-medium text-slate-200 disabled:opacity-40">{copied ? <Check size={15}/> : <Copy size={15}/>} {copied ? t.copied : t.copy}</button><button type="button" onClick={() => void compose()} className="min-h-12 rounded-xl border border-white/10 px-3 text-sm text-slate-300">{t.regenerate}</button><button type="button" onClick={() => void openWhatsApp(draft, phone)} disabled={!draft} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl bg-emerald-400 px-3 text-sm font-semibold text-slate-950 disabled:opacity-40"><MessageCircle size={16}/>{phone ? ui.directWhatsapp : ui.chooseWhatsapp}</button><button type="button" onClick={() => void markManualSent()} disabled={!draft} className="inline-flex min-h-12 items-center justify-center gap-2 rounded-xl border border-emerald-400/25 bg-emerald-400/[0.07] px-3 text-sm font-semibold text-emerald-200 disabled:opacity-40"><Check size={16}/>{currentLang === 'pt-BR' ? 'Marcar como enviado' : currentLang === 'es-ES' ? 'Marcar enviado' : 'Mark as sent'}</button></div>

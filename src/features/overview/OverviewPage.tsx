  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex items-center gap-2 mb-2">
         <span className="bg-amber-950/40 border border-amber-500/20 text-amber-300 px-2 py-0.5 rounded text-xs font-semibold uppercase">{t.demoDataBadge}</span>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 md:gap-4">
        ...
        <div className="bg-[#121824] border border-white/10 rounded-xl p-3 md:p-4 shadow-md min-w-0">
          <div className="flex items-center justify-between text-gray-400 mb-2">
            <span className="text-xs md:text-sm font-medium truncate">{t.kpiWaiting}</span>
          ...
          <div className="text-xs text-amber-300 mt-1 truncate">{t.requiresAttention}</div>
        </div>

        {/* KPI 3 */}
        <div className="bg-[#121824] border border-white/10 rounded-xl p-3 md:p-4 shadow-md min-w-0">
          <div className="flex items-center justify-between text-gray-400 mb-2">
            <span className="text-xs md:text-sm font-medium truncate">{t.kpiAHT}</span>
          ...
          <div className="text-xs text-emerald-400 mt-1 truncate">{t.less30s}</div>
        </div>

        {/* KPI 4 */}
        <div className="bg-[#121824] border border-white/10 rounded-xl p-3 md:p-4 shadow-md min-w-0">
          <div className="flex items-center justify-between text-gray-400 mb-2">
            <span className="text-xs md:text-sm font-medium truncate">{t.kpiAiResolution}</span>
          ...
          <div className="text-xs text-emerald-400 mt-1 truncate">{t.fullyAutonomous}</div>
        </div>

        {/* KPI 5 */}
        <div className="bg-[#121824] border border-white/10 rounded-xl p-3 md:p-4 shadow-md min-w-0 col-span-2 sm:col-span-1">
          <div className="flex items-center justify-between text-gray-400 mb-2">
            <span className="text-xs md:text-sm font-medium truncate">{t.kpiPlannedChannels}</span>
          ...
          <div className="text-xs text-amber-400 mt-1 truncate">{t.noActiveIntegrations}</div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6 min-w-0">
          ...
          <div className="bg-[#121824] border border-white/10 rounded-2xl overflow-hidden shadow-xl">
            <div className="p-4 md:p-5 border-b border-white/10 flex items-center justify-between">
              ...
              <button 
                className="text-xs text-indigo-400 hover:text-indigo-300 font-medium min-h-[44px] px-2"
                onClick={() => onNavigate('inbox')}
              >
                {t.viewAll}
              </button>
            ...
            <div className="divide-y divide-white/5">
              {[
                ...
                <button
                  type="button"
                  key={item.id}
                  className="w-full text-left p-4 hover:bg-white/5 transition cursor-pointer flex flex-col md:flex-row md:items-center justify-between gap-3 min-w-0 outline-none focus:bg-white/5 focus:ring-2 focus:ring-indigo-500/50"
                  onClick={() => onNavigate('inbox')}
                >
                  ...
                  <div className="flex flex-row md:flex-col items-center md:items-end justify-between gap-2 md:gap-0 mt-2 md:mt-0 border-t md:border-t-0 border-white/5 pt-2 md:pt-0">
                    <div className="flex items-center gap-1.5 px-2 py-0.5 rounded text-xs font-medium bg-amber-500/10 text-amber-400 border border-amber-500/20 max-w-full">
                      <span className="w-1.5 h-1.5 rounded-full bg-amber-400 shrink-0"></span>
                      <span className="whitespace-normal">{item.status}</span>
                    </div>
                    <div className="text-xs text-gray-500 mt-1 md:mt-0.5">{item.time}</div>
                  </div>
                </button>
              ))}
            </div>
          ...
          <div className="bg-[#121824] border border-white/10 rounded-2xl p-4 md:p-5 shadow-xl">
            ...
            <p className="text-xs text-gray-500 mb-6">{t.demoVolumeDesc}</p>
            <div className="h-48 flex items-end justify-between gap-1 md:gap-2 mt-4">
              {[40, 65, 45, 80, 55, 90, 75].map((height, i) => (
                <div key={i} className="flex-1 flex flex-col justify-end items-center gap-2">
                  <div
                    role="img"
                    aria-label={t.chartValueLabel(t.weekdaysShort[i], height * 12)}
                    ...
                    style={{ height: `${height}%` }}
                  >
                    <div className="opacity-0 group-hover:opacity-100 absolute -top-8 left-1/2 -translate-x-1/2 bg-[#222C42] border border-white/10 text-white text-xs px-2 py-1 rounded transition-opacity shadow-lg">
                      {height * 12}
                    </div>
                  </div>
          ...
          <div className="bg-[#121824] border border-white/10 rounded-2xl overflow-hidden shadow-xl">
            ...
            <div className="divide-y divide-white/5">
              {[
                {
                  id: 1,
                  action: t.createSubscriptionDemo,
                  app: 'NestFinance',
                  status: t.simulatedSuccess,
                  color: 'text-emerald-400',
                  bg: 'bg-emerald-500/10 border-emerald-500/20',
                  time: t.time12m,
                  detail: t.nestFinanceMemoryOnly,
                },
                {
                  id: 2,
                ...
                <div key={item.id} className="p-4 min-w-0">
                  <div className="flex flex-col md:flex-row justify-between md:items-start gap-2 min-w-0">
                    <div className="min-w-0">
                      <div className="text-sm font-semibold text-gray-200 break-words">{item.action}</div>
                  ...
                  <div className="mt-2 flex flex-col gap-1.5">
                    <div className={`inline-flex items-center self-start gap-1 px-2 py-0.5 rounded text-xs font-medium ${item.color} ${item.bg} border max-w-full`}>
                      <span className="whitespace-normal">{item.status}</span>
                    </div>
                    {item.detail && (
                      <div className="text-xs text-gray-400 break-words mt-1 leading-relaxed bg-white/5 p-2 rounded">
                        {item.detail}
                      </div>
                    )}
                  </div>
              ...
              <div className="p-4 flex flex-col gap-2 min-w-0">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 min-w-0">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-gray-500 truncate">+55 43 99990-7071</div>
                  </div>
                  <div className="inline-flex self-start md:self-auto items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-500/10 text-amber-400 border border-amber-500/20 max-w-full">
                    <span className="whitespace-normal">{t.planned}</span>
                  </div>
                </div>
                <div className="text-xs text-gray-500 bg-white/5 p-1.5 rounded">{t.coexistenceCloudApi}</div>
              </div>
              ...
              <div className="p-4 flex flex-col gap-2 min-w-0">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 min-w-0">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-gray-500 truncate">Direct Messages</div>
                  </div>
                  <div className="inline-flex self-start md:self-auto items-center px-2 py-0.5 rounded text-xs font-medium bg-gray-500/10 text-gray-400 border border-gray-500/20 max-w-full">
              ...
              <div className="p-4 flex flex-col gap-2 min-w-0">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 min-w-0">
                  <div className="min-w-0">
                    ...
                    <div className="text-xs text-gray-500 truncate">In-App Web/Mobile</div>
                  </div>
                  <div className="inline-flex self-start md:self-auto items-center px-2 py-0.5 rounded text-xs font-medium bg-amber-500/10 text-amber-400 border border-amber-500/20 max-w-full">
                    <span className="whitespace-normal">{t.planned}</span>
                  </div>
                </div>
                <div className="text-xs text-gray-500 bg-white/5 p-1.5 rounded">{t.futureIntegration}</div>
              </div>
            </div>
            <div className="p-3 border-t border-white/10 bg-white/[0.02]">
              <button 

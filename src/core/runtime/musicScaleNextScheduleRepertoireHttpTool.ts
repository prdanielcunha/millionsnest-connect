...
export const MUSIC_SCALE_NEXT_REPERTOIRE_PATH =
...
export interface MusicScaleNextScheduleRepertoireHttpToolOptions {
  musicScaleOrigin: string;
  fetchImpl?: ToolFetch;
  timeoutMs?: number;
}

type MusicScaleRepertoirePayload = {
  success: boolean;
  code?: string;
  auditId?: string;
...
function isPayload(value: unknown): value is MusicScaleRepertoirePayload {
  return Boolean(
    value &&
      typeof value === 'object' &&
      'success' in value &&
...
export class MusicScaleNextScheduleRepertoireHttpTool {
  private readonly endpoint: string;
  private readonly fetchImpl: ToolFetch;
  private readonly timeoutMs: number;

  constructor(options: MusicScaleNextScheduleRepertoireHttpToolOptions) {
    const origin = normalizeOrigin(options.musicScaleOrigin);
    this.endpoint = new URL(MUSIC_SCALE_NEXT_REPERTOIRE_PATH, origin).toString();
    this.fetchImpl =
      options.fetchImpl ??
  ...
  async getNextScheduleRepertoire(
    ...
    try {
      ...
      if (response.status === 401 || response.status === 403) {
        return {
          ...
          humanSummary: safeSummary(
            ...
            'O MusicScale negou acesso a esse repertório.',
          ),
      ...
      if (!response.ok) {
        return {
          status: 'failed',
          humanSummary: 'Não consegui consultar o repertório do MusicScale agora.',
          auditId: isPayload(payload) ? payload.auditId : undefined,
      ...
      const repertoire = Array.isArray(payload.repertoire)
        ? payload.repertoire.filter((item) => item && typeof item === 'object').slice(0, 100)
        : [];
      ...
      return {
        ...
        humanSummary: safeSummary(
          ...
          payload.schedule
            ? 'Encontrei o repertório da sua próxima escala no MusicScale.'

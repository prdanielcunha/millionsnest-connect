...
function deriveRadarPeople(input) {
  ...
  const people = [];
  for (const [normalizedName2, messages] of grouped.entries()) {
    const ordered = [...messages].sort((a, b) => a.index - b.index);
    const displayName = ordered[ordered.length - 1]?.sender || normalizedName2;
    const personId = stableId("person", normalizedName2);
    const signals = [];
...
function greeting(name, style) {
  ...
  if (style === "profissional") return name ? `Ol\xE1, ${name}. Espero que esteja bem.` : "Ol\xE1. Espero que esteja bem.";
  if (style === "descontraido") return name ? `Fala, ${name}! Tudo certo?` : "Fala! Tudo certo?";
  if (style === "objetivo") return name ? `${name},` : "Ol\xE1,";
  if (style === "proximo") return name ? `Oi, ${name}! Tudo certo por a\xED?` : "Oi! Tudo certo por a\xED?";
  if (style === "pastoral") return name ? `Ol\xE1, ${name}! Espero que esteja tudo bem com voc\xEA e com a igreja.` : "Ol\xE1! Espero que esteja tudo bem por a\xED na igreja.";
  if (style === "consultivo") return name ? `Oi, ${name}! Quero entender um pouco melhor a realidade de voc\xEAs.` : "Oi! Quero entender um pouco melhor a realidade de voc\xEAs.";
  return `Oi${suffix}! Tudo bem?`;
}
function soften(style, text) {
  let result = text;
  if (style === "amigavel") {
    return result.replace(/Posso te fazer uma pergunta rápida\?/g, "Me conta uma coisa?").replace(/Posso te fazer uma pergunta rapidinha\?/g, "Me conta uma coisa?").replace(/acho que faz sentido/g, "acho que pode te ajudar").replace(/O que mais dá trabalho/g, "O que mais pesa na rotina");
  }
  if (style === "profissional") {
    return result.replace(/\bA gente\b/g, "N\xF3s").replace(/\ba gente\b/g, "n\xF3s").replace(/acho que faz sentido/g, "acredito que vale a pena").replace(/Posso te mandar/g, "Posso lhe enviar").replace(/posso te mandar/g, "posso lhe enviar").replace(/te mostrar/g, "lhe mostrar").replace(/te explico/g, "lhe explico").replace(/rapidinho/g, "brevemente").replace(/curtinho/g, "breve").replace(/vídeo bem rápido/g, "v\xEDdeo breve").replace(/equipe daí/g, "equipe da sua igreja");
  }
  if (style === "descontraido") {
    return result.replace(/Posso te fazer uma pergunta rápida\?/g, "Me diz uma coisa:").replace(/Posso te fazer uma pergunta rapidinha\?/g, "Me diz uma coisa:").replace(/Em vez de te explicar tudo por texto/g, "Pra n\xE3o ficar naquele text\xE3o").replace(/Se você quiser/g, "Se quiser").replace(/Você conseguiu ver com calma\?/g, "Deu pra dar uma olhada?");
  }
  if (style === "objetivo") {
    return result.replace(/Espero que esteja bem\.\s*/g, "").replace(/Tudo bem\?\s*/g, "").replace(/Tudo certo\?\s*/g, "").replace(/Tudo certo por aí\?\s*/g, "").replace(/Queria te fazer uma pergunta rapidinha:/g, "Pergunta direta:").replace(/Posso te fazer uma pergunta rápida\?/g, "Pergunta direta:").replace(/Pelo que você comentou sobre/g, "Sobre").replace(/Pensando no que você falou sobre/g, "Sobre").replace(/acho que faz sentido te mostrar uma coisa\./g, "vale te mostrar isso.").replace(/Em vez de te explicar tudo por texto,/g, "").replace(/\s{2,}/g, " ").trim();
  }
  if (style === "proximo") {
    return result.replace(/Pelo que você comentou sobre/g, "Lembrei do que voc\xEA comentou sobre").replace(/Pensando no que você falou sobre/g, "Lembrei do que voc\xEA falou sobre").replace(/Passando só para retomar/g, "Lembrei de voc\xEA e quis retomar").replace(/Se fizer sentido/g, "Se isso fizer sentido pra voc\xEA").replace(/Quer que eu te mostre/g, "Quer que eu te mostre do jeito mais simples");
  }
  if (style === "pastoral") {
    return result.replace(/acho que faz sentido/g, "creio que pode fazer sentido").replace(/Pensando na rotina de vocês/g, "Pensando com cuidado na rotina da igreja de voc\xEAs").replace(/Se você pudesse resolver só uma parte/g, "Pensando na realidade da igreja, se voc\xEAs pudessem resolver uma parte").replace(/Quer que eu te mostre/g, "Se fizer sentido para voc\xEAs, posso mostrar").replace(/O teste ajudou de verdade/g, "O teste serviu bem \xE0 rotina de voc\xEAs");
  }
  if (style === "consultivo") {
    return result.replace(/Posso te fazer uma pergunta rápida\?/g, "Para eu entender melhor:").replace(/Posso te fazer uma pergunta rapidinha\?/g, "Para eu entender melhor:").replace(/O que mais dá trabalho/g, "Qual \xE9 hoje o maior gargalo").replace(/Se você pudesse resolver só uma parte/g, "Se tivesse que priorizar um \xFAnico ponto").replace(/acho que faz sentido te mostrar uma coisa\./g, "pelo cen\xE1rio que voc\xEA trouxe, vale avaliar uma alternativa.").replace(/Quer que eu te mostre especificamente essa parte\?/g, "Faz sentido eu te mostrar somente essa parte para avaliarmos se resolve o problema?").replace(/Ficou alguma dúvida ou alguma parte que você queria ver melhor\?/g, "Qual ponto ainda precisa ficar mais claro para voc\xEA avaliar se isso resolve a necessidade?");
  }
  return result;
}
...
function videoSendVariants(name, signal, style) {
  const g = greeting(name, style);
  const t = topic(signal);
  return [
    `${g} Como combinamos, vou te mandar um v\xEDdeo bem curto mostrando justamente a parte de ${t}. Depois me diz se isso faria diferen\xE7a a\xED para voc\xEAs.`,
    `${g} Separei um v\xEDdeo r\xE1pido do MusicScale focado em ${t}. Assiste quando puder e depois quero saber qual parte mais conversa com a realidade de voc\xEAs.`,
    `${g} Te mando agora uma demonstra\xE7\xE3o curtinha. Repara principalmente na parte de ${t}; depois me fala se hoje isso ajudaria a equipe da\xED.`
  ].map((text) => soften(style, text));
}
function diagnosticVariants(name, signal, style) {
  const g = greeting(name, style);
  const t = topic(signal);
  return [
    `${g} Do que voc\xEA viu, qual parte faria mais diferen\xE7a a\xED hoje: ${t}, a organiza\xE7\xE3o da equipe ou as confirma\xE7\xF5es?`,
    `${g} Pensando na rotina de voc\xEAs, o que mais te chamou aten\xE7\xE3o no v\xEDdeo? Quero entender antes de te mostrar qualquer outra coisa.`,
    `${g} Se voc\xEA pudesse resolver s\xF3 uma parte da organiza\xE7\xE3o do louvor agora, qual seria?`
  ].map((text) => soften(style, text));
}
function videoScriptVariants(name, signal, style) {
  const t = topic(signal);
  const who = name ? `, ${name}` : "";
  return [
    `Oi${who}! Gravei rapidinho porque \xE9 mais f\xE1cil te mostrar. O MusicScale nasceu da nossa pr\xF3pria rotina de igreja. Aqui a equipe recebe a escala, v\xEA repert\xF3rio, cifras e tons e confirma presen\xE7a sem depender de mensagem perdida. Pensando no que voc\xEA comentou sobre ${t}, olha como essa parte fica organizada.`,
    `Oi${who}! Em menos de meio minuto eu quero te mostrar s\xF3 uma coisa. A gente tinha muita informa\xE7\xE3o espalhada no WhatsApp e criou o MusicScale para centralizar a rotina do louvor. Repara especialmente nessa parte de ${t}, porque foi exatamente o ponto que lembrei da nossa conversa.`,
    `Oi${who}! Vou te mostrar sem apresenta\xE7\xE3o comercial, s\xF3 na pr\xE1tica. Aqui est\xE1 uma escala real: equipe, m\xFAsicas, cifras, tons e confirma\xE7\xE3o num lugar s\xF3. Pelo que voc\xEA falou sobre ${t}, acho que essa \xE9 a parte que mais vale voc\xEA olhar primeiro.`
  ].map((text) => soften(style, text));
}
...
function buildComposerPlan(input) {
  ...
  const recommendation = stage <= 2 ? "Comece com uma pergunta curta. N\xE3o apresente o MusicScale inteiro ainda." : stage === 3 ? "Conte a hist\xF3ria em 35\u201350 segundos, como conversa. Termine pedindo permiss\xE3o para mostrar." : stage === 4 ? "Pe\xE7a permiss\xE3o antes de mandar o v\xEDdeo. O pr\xF3ximo passo \xE9 um pequeno \u201Csim\u201D." : stage === 5 ? "Mostre s\xF3 o necess\xE1rio em cerca de 30 segundos e conecte a demonstra\xE7\xE3o \xE0 dor real da conversa." : stage === 6 ? "Depois do v\xEDdeo, fa\xE7a uma pergunta diagn\xF3stica. N\xE3o continue apresentando recursos sem ouvir a resposta." : stage === 7 ? `Fale somente da parte ligada a ${t}; n\xE3o despeje todos os recursos.` : stage === 8 ? "S\xF3 convide para o trial quando houver inten\xE7\xE3o real. Use uma rotina da pr\xF3pria igreja." : stage === 10 ? "Pergunte primeiro se facilitou. S\xF3 depois apresente continuidade e plano vigente." : "Avance uma etapa por vez e adapte a pr\xF3xima mensagem \xE0 resposta real.";
...
var SALES_STAGES = /* @__PURE__ */ new Set([
  "iniciar_conversa",
  "descobrir_dor",
  "contar_historia",
  "pedir_video",
...
var COMMERCIAL_ACTIONS = /* @__PURE__ */ new Set(["whatsapp_opened", "sent_manual", "copied"]);
var MESSAGE_MODEL_TONES = /* @__PURE__ */ new Set(["curto", "conversa", "audio", "video"]);
function resolveFollowUpDays(value) {
  const days = value === void 0 ? 2 : Number(value);
  if (!Number.isInteger(days) || days < 1 || days > 90) throw new Error("FOLLOW_UP_DAYS_INVALID");
  return days;
}
function isFollowUpDue(person, nowMs) {
...
var PersonalRadarService = class {
  ...
  async getRadar(request) {
    ...
    const active = people.filter((person) => person.radarEligible !== false && person.radarState !== "ignored" && !isActivelySnoozed(person, nowMs)).sort((a, b) => {
  ...
  async getPeople(request) {
    const context = await this.resolvePilotContext(request);
    const people = await this.vault.list(request.authToken, context.actorUid, ["personalPeople"], 2e3);
  ...
  async importContacts(request, input) {
    const context = await this.resolvePilotContext(request);
    ...
    const existing = await this.vault.list(request.authToken, context.actorUid, ["personalPeople"], 2e3);
    const writes = [];
    let imported = 0;
    for (const raw of contacts) {
      ...
      const id = person ? String(person.id) : `ct_${import_node_crypto3.default.createHash("sha256").update(`${nameKey}|${phone}`).digest("hex").slice(0, 24)}`;
      const currentKinds = Array.isArray(person?.sourceKinds) ? person.sourceKinds : String(person?.sourceId || "").startsWith("wa_") ? ["whatsapp_export"] : [];
      const sourceKinds = Array.from(/* @__PURE__ */ new Set([...currentKinds, "contacts_import"]));
      const directSignal = {
        id: "direct_contact",
        type: "recurring_relevant_topic",
        reason: "Contato dispon\xEDvel para abordagem direta.",
        nextAction: "Escolha a etapa da conversa e prepare a pr\xF3xima mensagem.",
        evidence: []
      };
      writes.push({ path: ["personalPeople", id], data: {
        ...person || {},
        id,
        sourceId: String(person?.sourceId || "contacts_import"),
        ownerUid: context.actorUid,
        displayName,
        phone: phone || person?.phone || null,
        sourceKinds,
        signals: Array.isArray(person?.signals) && person.signals.length ? person.signals : [directSignal],
        radarEligible: person?.radarEligible === void 0 ? false : person.radarEligible,
        radarState: String(person?.radarState || "active"),
        priority: Number(person?.priority ?? 99),
        salesStage: person?.salesStage || "iniciar_conversa",
        importedAt: person?.importedAt || isoNow(this.now),
  ...
  async updatePerson(request, personDocumentId, input) {
    ...
    if (input.salesStage !== void 0 && !SALES_STAGES.has(input.salesStage)) throw new Error("SALES_STAGE_INVALID");
    if (input.commercialAction !== void 0 && !COMMERCIAL_ACTIONS.has(input.commercialAction)) throw new Error("COMMERCIAL_ACTION_INVALID");
  ...
  async listMessageModels(request) {
    const context = await this.resolvePilotContext(request);
    const models = await this.vault.list(request.authToken, context.actorUid, ["messageModels"], 100);
    const ordered = models.filter((model) => typeof model.text === "string" && typeof model.label === "string").sort((a, b) => String(b.createdAt || "").localeCompare(String(a.createdAt || ""))).slice(0, 30);
    return { models: ordered, count: ordered.length };
  }
  async saveMessageModel(request, input) {
    const context = await this.resolvePilotContext(request);
    const text = typeof input.text === "string" ? input.text.trim() : "";
    if (!text || text.length > 4e3) throw new Error("MESSAGE_MODEL_TEXT_INVALID");
    ...
    const createdAt = isoNow(this.now);
    const id = `mdl_${import_node_crypto3.default.createHash("sha256").update(`${context.actorUid}|${createdAt}|${text}|${import_node_crypto3.default.randomUUID()}`).digest("hex").slice(0, 24)}`;
    const model = {
      id,
      ownerUid: context.actorUid,
      label,
    ...
    const excess = [...existing].sort((a, b) => String(a.createdAt || "").localeCompare(String(b.createdAt || ""))).slice(0, Math.max(0, existing.length - 29));
    if (excess.length) {
  ...
  async deleteMessageModel(request, modelId) {
    const context = await this.resolvePilotContext(request);
...
function humanSummary(error) {
  ...
  if (code === "CONTACTS_REQUIRED") return "Adicione pelo menos um contato v\xE1lido.";
  ...
  if (code === "SALES_STAGE_INVALID" || code === "COMMERCIAL_ACTION_INVALID") return "O estado comercial informado \xE9 inv\xE1lido.";
  if (code.startsWith("MESSAGE_MODEL_")) return "N\xE3o foi poss\xEDvel salvar este modelo de mensagem.";
...
function createPersonalRadarRouter(service) {
  ...
  router.post("/imports/contacts", import_express.default.json({ limit: "512kb" }), execute(async (req, res) => {
    const body = req.body;
    ...
    execute(async (req, res) => {
      ...
      const result = await service.updatePerson(
        ...
        safeId(req.params.personId),
        {
          ...
          salesStage: typeof body.salesStage === "string" ? body.salesStage : void 0,
          commercialAction: typeof body.commercialAction === "string" ? body.commercialAction : void 0,
          followUpDays: typeof body.followUpDays === "number" ? body.followUpDays : void 0
        }
      );
  ...
  router.post("/message-models", import_express.default.json({ limit: "32kb" }), execute(async (req, res) => {
    const body = req.body;
